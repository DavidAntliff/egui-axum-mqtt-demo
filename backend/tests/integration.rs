...
use backend::{
    build_router, create_mqtt, create_state, spawn_mqtt_loop, PingPayload, TOPIC_LIVE, TOPIC_PING_REQ,
    TOPIC_PING_RESP, TOPIC_POLL, TOPIC_SEND,
};

/// A self-contained test environment: starts the backend on an ephemeral port,
/// creates a separate MQTT client for the "test side" (simulating the outside
/// world / device), and provides helpers for WebSocket and HTTP interaction.
struct TestHarness {
    addr: SocketAddr,
    http: reqwest::Client,
...
impl TestHarness {
    ...
    ///
    /// `test_subscriptions` — MQTT topics the test side should subscribe to
    /// so that we can observe what the backend publishes.
    async fn new(test_subscriptions: &[&str]) -> Self {
        // Backend side:
        // Each test gets a unique client-id to avoid collisions when tests run in parallel.
        let id = uuid_short();
        let (mqtt_client, eventloop) =
            create_mqtt(&format!("test-backend-{id}"), "localhost", 1883).await;
        ...
        for topic in test_subscriptions {
...
async fn pattern1_realtime_send() {
    ...
    let mut h = TestHarness::new(&[TOPIC_SEND]).await;
    ...
    let payload = h.expect_mqtt_on_topic(TOPIC_SEND, T).await;
...
async fn pattern2_poll_returns_last_mqtt_message() {
    ...
    h.test_mqtt
        .publish(TOPIC_POLL, QoS::AtLeastOnce, false, b"poll-value-1")
        .await
        .unwrap();
    ...
    assert_eq!(body.topic, TOPIC_POLL);
    assert_eq!(body.payload, "poll-value-1");

    // A second publish should overwrite the cached value.
    h.test_mqtt
        .publish(TOPIC_POLL, QoS::AtLeastOnce, false, b"poll-value-2")
        .await
        .unwrap();
    tokio::time::sleep(Duration::from_millis(500)).await;
...
async fn pattern3_realtime_receive() {
    ...
    h.test_mqtt
        .publish(TOPIC_LIVE, QoS::AtLeastOnce, false, b"sensor-reading-42")
        .await
        .unwrap();
...
async fn pattern4_ping_device_roundtrip() {
    ...
    let mut h = TestHarness::new(&[TOPIC_PING_REQ]).await;
    let mut ws = h.connect_ws().await;
    ...
    let req_bytes = h.expect_mqtt_on_topic(TOPIC_PING_REQ, T).await;
    let req: PingPayload =
    ...
    .unwrap();
    h.test_mqtt
        .publish(TOPIC_PING_RESP, QoS::AtLeastOnce, false, reply)
        .await
        .unwrap();
...
async fn broadcast_reaches_multiple_ws_clients() {
    ...
    h.test_mqtt
        .publish(TOPIC_LIVE, QoS::AtLeastOnce, false, b"broadcast-msg")
        .await
        .unwrap();
...
async fn bad_ws_message_does_not_crash() {
    ...
    h.test_mqtt
        .publish(TOPIC_LIVE, QoS::AtLeastOnce, false, b"still-alive")
        .await
        .unwrap();
